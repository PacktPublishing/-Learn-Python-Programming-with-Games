...
horse_img_list = [pygame.image.load(path.join('images', 'cheval' + str(x) + '.png')) for x in range(1, 9)]
horses_in_list = len(horse_img_list)
...
def run_game():
    ...
    while run_game:
        ...
        if pressed_keys[K_SPACE]: 
            ...
            if guard_img_idx >= 9:
                guard_img_idx = 0
            guard.player_img = guard.sprite_list[guard_img_idx]
            
        if pressed_keys[K_RIGHT]:
            horse_rider.player_img_rect.x += speed
            horse_img_idx += 1
            if horse_img_idx >= horses_in_list:
                horse_img_idx = 0
            horse_rider.player_img = horse_img_list[horse_img_idx]           
        ...
        if pressed_keys[K_w]: guard.player_img_rect.y -= speed
        if pressed_keys[K_s]: guard.player_img_rect.y += speed    
        if pressed_keys[K_a]: guard.player_img_rect.x -= speed    
        if pressed_keys[K_d]: guard.player_img_rect.x += speed
