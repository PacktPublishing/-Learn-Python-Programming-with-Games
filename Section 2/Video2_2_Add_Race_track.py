...
class GameArea(turtle.RawTurtle):               # inherent/extend RawTurtle
    ...
    def draw_race_track(self):
        ...
        race_track_pen.setundobuffer(None)                             # no undo buffer to speed up the drawing
        race_track_pen.hideturtle()
        race_track_pen.speed(0)                                        # 0 is the fastest
        race_track_pen.color('white', 'black')                         # pencolor, fillcolor
        race_track_pen.penup()                                         # don't draw while moving into position
        race_track_pen.setpos(GAME_AREA_START_X + 70, 
                              GAME_AREA_START_Y + 20)                  # left bottom
        race_track_pen.pendown()                                       # now start drawing
        race_track_pen.pensize(3)    

        race_track_pen.begin_fill()                    # start filling in the game area
        ...
        race_track_pen.setpos(GAME_AREA_START_X + 110, 
                              GAME_AREA_START_Y + 60)                  # left bottom        
        race_track_pen.pendown()                                       # now start drawing
